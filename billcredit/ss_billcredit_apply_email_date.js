...
function billcredit_apply_email_date(type) {
    ...
    var date2apply = nlapiGetContext().getSetting('SCRIPT', 'custscript_date2apply');
    var internal_email = nlapiGetContext().getSetting('SCRIPT', 'custscript_email_recipient');
    
    nlapiLogExecution('DEBUG','Date to Apply', date2apply);

    var filters1 = new Array();
    filters1[0] =  new nlobjSearchFilter('custbody_ts_bill_credit_apply_dd', null, 'on', date2apply, null);
	var rs2 = nlapiSearchRecord('transaction','customsearch_ts_vbillcredit_appliedtoday', filters1, null);
	
	if (isEmpty(rs2)) {
		dLog(LOG_NAME, 'No Credit Memos applied on specified date. Exit script.');
		return;
	}
	...
	for (var xx = 0; xx < rs2.length; xx++) {
      ...
      dLog(LOG_NAME, 'vendorcredit number : ' + rs2[xx].getValue('tranId') + 'vendor: ' + rs2[xx].getValue('name'));
	}
	...
	for (var xy in vendor2email) {
	    ...
	    filters3[1] =  new nlobjSearchFilter('custbody_ts_bill_credit_apply_dd', null, 'on', date2apply, null);
		var rs3 = nlapiSearchRecord('transaction','customsearch_ts_vbcredit_lines_app_date',filters3,null);
	    if (isEmpty(rs3)){
        ...
        if ( rs3.length > 0 && rs3 != null ) {
            ...
            for ( var j = 0; j < rs3.length; j++ ) {
            	...
            	vbc_email_table  += "<td>" + rs3[j].getValue('appliedtotransaction') + "</td>";
...
//		nlapiSubmitFile(file);
		...
		var bcc_email = 'pandora@threesixtysourcing.com';
        var bcc_email2 = 'ocs_support@yahoo.com';
		nlapiSendEmail(email_sender_id, contact_emails, emailSubj,"<html><body>"+emailBody+"</body></html>", null, bcc_email, null, file);
		nlapiSendEmail(email_sender_id, internal_email, emailSubj,"<html><body>"+emailBody+"</body></html>", null, bcc_email2, null, null);
