...
function testASN_Interface() {
	var reprocessData = nlapiGetFieldValue('custrecord_asn_data');
...
function processASN(data){
	...
	try{
		...
		// BEGIN ASN SETUP
		if(asnRecId){
			var asnRec = nlapiLoadRecord('customrecord_ts_asn', asnRecId, {recordmode: 'dynamic'});
		}
		else{
			var asnRec = nlapiCreateRecord('customrecord_ts_asn', {recordmode: 'dynamic'});
		}

		var filters = [];
		filters[0] = new nlobjSearchFilter('entityid', null, 'is', data.VENDOR_NAME);
		filters[1] = new nlobjSearchFilter('subsidiary', null, 'is', 6);
		...
		var firstFactId = recordLookup('customrecord_fty_profile', new nlobjSearchFilter('externalid', null, 'is', data.ASN_LINE_LEVEL[0].PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.FACTORY_ID), data.ASN_LINE_LEVEL[0].PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.FACTORY_ID);
		...
		asnRec.setFieldValue('custrecord_asn_commercial_inv', data.CPL_COMMERCIAL_INVOICE_NO);
		...
		for(var i = 0; i < attachments.length; i++){
			...
			var fileId = (curAttachment.NS_FILE_PATH) ? curAttachment.NS_FILE_PATH : 4424;
		...
		for(var i = 0; i < asnLines.length; i++){
			...
			var htsCode = (curLine.PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.BLANKET_PO_LINE.HTS_CODE) ? curLine.PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.BLANKET_PO_LINE.HTS_CODE : 'Other';
			...
			projRec.setFieldValue('externalid', projObj.CUSTOMER_PO_NUMBER);
			...
			projRec.setFieldValue('subsidiary', '6'); // Hard-coded internal ID reference.
			...
			if(curLine.PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.hasOwnProperty('COMPOSITE') === true && curLine.PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.COMPOSITE.COMPONENT_LEVEL.length > 0){
				...
				compositeItemRec.setFieldValue('custitem_ts_item_primary_upc', itemObj.PRIMARY_UPC);
				...
				compositeItemRec.setFieldValue('custitem_ts_item_hts_code', htsCodeId);
				...
				for(var j = 0; j < components.length; j++){
					...
					if(componentObj.ITEM_NO === itemObj.ITEM_NO){
						...
						componentItemRec.setFieldValue('custitem_ts_item_primary_upc', itemObj.PRIMARY_UPC);
						...
						componentItemRec.setFieldValue('custitem_ts_item_hts_code', htsCodeId);
						...
						componentItemRec.setFieldValue('incomeaccount', '2786');
						componentItemRec.setFieldValue('assetaccount', '1977');
						componentItemRec.setFieldValue('cogsaccount', '2905');
				...
				if(recType == 'lotnumberedinventoryitem'){
					...
					// Set Item Account Codes - hardcode for now - by Herman
					itemRec.setFieldValue('incomeaccount', '2786');
					itemRec.setFieldValue('assetaccount', '1977');
					itemRec.setFieldValue('cogsaccount', '2905');
				}
				else{
				...
				itemRec.setFieldValue('custitem_ts_item_primary_upc', itemObj.PRIMARY_UPC);
				...
				itemRec.setFieldValue('custitem_ts_item_hts_code', htsCodeId);
			...
			var factoryId = recordLookup('customrecord_fty_profile', new nlobjSearchFilter('externalid', null, 'is', bpoObj.FACTORY_ID), bpoObj.FACTORY_ID);
			...
			bpoLineRec.setFieldValue('custrecord_ts_bpol_hts_code', htsCodeId);
			...
			var rspoNameDupe = rspoName + '-' + curLine.ASN_LINE_NUM;

			var filterExpression = [[['tranid', 'is', rspoName], 'or', ['tranid', 'is', rspoNameDupe]], 'and', ['mainline', 'is', 'T']];
			...
			if(rspoRecId){
				filters[0] = new nlobjSearchFilter('externalid', null, 'is', rspoNameDupe);
				...
				if(rspoRecId){
					...
					var relatedASN = rspoRec.getFieldValue('custbody_ts_rspo_related_asn');

					if(relatedASN != asnRecId){
						interfaceWarning += '<BR><B style="COLOR: orange">WARNING:</B> The same PO already exists for another ASN.';
						filters[0] = new nlobjSearchFilter('externalid', null, 'is', (rspoNameDupe + '-' + data.BATCH_PROCESS_ID));
						filters[1] = new nlobjSearchFilter('mainline', null, 'is', 'T');
						rspoRecId = recordLookup('purchaseorder', filters, (rspoNameDupe + '-' + data.BATCH_PROCESS_ID), null, 'T');
						...
						rspoRec.setFieldValue('tranid', rspoNameDupe + '-' + data.BATCH_PROCESS_ID);
						rspoRec.setFieldValue('externalid', rspoNameDupe + '-' + data.BATCH_PROCESS_ID);
					}
					else{
						var curTranId = rspoRec.getFieldValue('tranid');
						rspoRec.setFieldValue('custbodyts_rspo_bpo_no', bpoRecId);
						rspoRec.setFieldValue('entity', bpoVendor);
						rspoRec.setFieldValue('tranid', curTranId);
			...
			rspoRec.setFieldValue('ustbody_ts_rspo_ms_release_item_desc', rspoLineObj.MS_RELEASE_ITEM_DESC);
			...
			rspoRec.setCurrentLineItemText('item', 'custcol_ts_rspo_needbydate', formatDate(rspoLineObj.NEED_BY_DATE));
			...
			rspoRec.setCurrentLineItemValue('item', 'custcol_ts_repo_need_qc_inspection', (rspoLineObj.NEED_QC_INSPECTION === 'Y') ? 'T' : 'F');
			...
			asnLineRec.setFieldValue('externalid', asnLineName);
			...
			var containerObj = curLine.CONTAINER_LEVEL;
									...
									if(containerNodeData.nodeType === 1 && containerNodeData.firstChild !== null){
										...
										while(containerData !== null){
											if(containerData.nodeType === 1){
												containers[containerData.nodeName] = (containerData.firstChild === null) ? '' : containerData.firstChild.nodeValue;
											}
