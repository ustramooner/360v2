    ...
    try {
        ...
        if (reprocessData) {
            ...
            for (var i = 0; i < asnArray.length; i++) {
                ...
                nlapiLogExecution('DEBUG', 'Single ASN', JSON.stringify(singleASN));
        ...
        nlapiLogExecution('ERROR', 'ERROR', 'Error while processing XML document');
...
function processASN(data) {
    ...
    try {
        ...
        // ADDED by Clemen - For debugging purposes only
        var jsonFile = nlapiCreateFile(data.ASN_NO + '.json', 'PLAINTEXT', JSON.stringify(data));
        jsonFile.setEncoding('UTF-8');
        jsonFile.setFolder(85);
        var jsonFileId = nlapiSubmitFile(jsonFile);

        nlapiSendEmail(63979, 'ccanaria@gmail.com', data.ASN_NO, 'Please see attached file.', null, null, null, jsonFile);
        // END by Clemen - For debugging purposes only
        ...
        //  nlapiLogExecution('DEBUG', 'interfaceRecId', interfaceRecId);
        ...
        //  nlapiLogExecution('DEBUG', "interfaceRec.getFieldValue('custrecord_asn_rec_ref')", interfaceRec.getFieldValue('custrecord_asn_rec_ref'));
        ...
        //  nlapiLogExecution('DEBUG', 'asnRec', JSON.stringify(asnRec));
        ...
        if (asnBill === null || asnInvoice === null) {
            ...
            nlapiLogExecution('DEBUG', 'Looking for ASN_NO', data.ASN_NO);
            ...
            var forwarder = null;
            if (data.hasOwnProperty('FORWARDER') === true) {
                forwarder = recordLookup('customrecord_ts_asn_forwarder_list', new nlobjSearchFilter('name', null, 'is', data.FORWARDER), data.FORWARDER);
            }

            // BEGIN ASN SETUP
            if (asnRecId) {
                asnRec = nlapiLoadRecord('customrecord_ts_asn', asnRecId, {
                    recordmode: 'dynamic'
                });
            } else {
                nlapiLogExecution('DEBUG', 'Creating New ASN', asnRecId);
                asnRec = nlapiCreateRecord('customrecord_ts_asn');
                //nlapiLogExecution('DEBUG', 'START ASN', 'START ASN 2- ' + asnRec);
                asnRecId = nlapiSubmitRecord(asnRec, false, true);
                asnRec = nlapiLoadRecord('customrecord_ts_asn', asnRecId, {
                    recordmode: 'dynamic'
                });
            }
            ...
            var filters = [];
            filters[0] = new nlobjSearchFilter('entityid', null, 'is', parseToString(data.VENDOR_NAME));
            filters[1] = new nlobjSearchFilter('subsidiary', null, 'is', globeVar.subsidiary);
            var asnVendor = recordLookup('vendor', filters, parseToString(data.VENDOR_NAME));
            ...
            // START Added by Clemen 22 Sept 2017
            if (!(data.ASN_LINE_LEVEL instanceof Array)) {
                data.ASN_LINE_LEVEL = [data.ASN_LINE_LEVEL];
            }
            // END Added by Clemen 22 Sept 2017
            ...
            nlapiLogExecution('DEBUG', 'START ASN', 'START ASN - ' + asnRec.getFieldValue('name'));
            if (data.hasOwnProperty('ASN_TYPE')) {
            ...
            if (data.hasOwnProperty('CONSIGNEE')) {
                asnRec.setFieldValue('custrecord_asn_consignee', recordLookup('customrecord_ts_asn_consignee_list', new nlobjSearchFilter('name', null, 'is', data.CONSIGNEE), data.CONSIGNEE, 'T'));
            }
            ...
            if (data.hasOwnProperty('TEAM_NAME')) {
                asnRec.setFieldValue('custrecord_asn_team', recordLookup('customrecord_ts_team_list', new nlobjSearchFilter('name', null, 'is', data.TEAM_NAME), data.TEAM_NAME, 'T'));
            }
            if (data.hasOwnProperty('TRANSPORTATION_MODE')) {
            ...
            if (data.hasOwnProperty('PORT_OF_LOADING')) {
                asnRec.setFieldValue('custrecord_asn_port_of_loading', recordLookup('customrecord_ts_asn_pol_list', new nlobjSearchFilter('name', null, 'is', data.PORT_OF_LOADING), data.PORT_OF_LOADING, 'T'));
            }
            if (data.hasOwnProperty('PORT_OF_DISCHARGE')) {
                asnRec.setFieldValue('custrecord_asn_port_of_discharge', recordLookup('customrecord_ts_rspo_asn_pod_list', new nlobjSearchFilter('name', null, 'is', data.PORT_OF_DISCHARGE), data.PORT_OF_DISCHARGE, 'T'));
            }
            ...
            if (data.hasOwnProperty('SHIPPING_DOC_TYPE')) {
            ...
            for (var i = 0; i < attachments.length; i++) {
                ...
                nlapiLogExecution('DEBUG', 'FileId and Record Id', fileId + ' ' + asnRecId);
                if (isNaN(fileId)) {
                    interfaceError += '<BR><B style="COLOR: red">ERROR:</B> ASN Attachment reference is not a number.';
                    interfaceRec.setFieldValue('custrecord_interface_complete', 'F');
                    interfaceRec.setFieldValue('custrecord_has_error', 'T');
                }else if (isFileExists(fileId) == false) {
                    throw 'File with id ' + fileId + ' does not exist in the file cabinet';
                   // interfaceRec.setFieldValue('custrecord_interface_complete', 'F');
                   // interfaceRec.setFieldValue('custrecord_has_error', 'T');
                } else {
            ...
            for (var i = 0; i < asnLines.length; i++) {
                ...
                nlapiLogExecution('DEBUG', 'projRecId', projRecId);

                // If Project does not exist, create a new one
                var projRec = (projRecId) ? nlapiLoadRecord('job', projRecId, {
                    recordmode: 'dynamic'
                }) : nlapiCreateRecord('job', {
                    recordmode: 'dynamic'
                });

                nlapiLogExecution('DEBUG', 'projObj.TEAM_NAME', projObj.TEAM_NAME);

                var projTeam = recordLookup('customrecord_ts_team_list', new nlobjSearchFilter('name', null, 'is', projObj.TEAM_NAME), projObj.TEAM_NAME, 'T');
                nlapiLogExecution('DEBUG', 'projTeam', projTeam);
                ...
                if (curLine.PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.hasOwnProperty('COMPOSITE') === true && curLine.PURCHASE_ORDER.PURCHASE_ORDER_LINE.BLANKET_PO_HEADER.COMPOSITE.COMPONENT_LEVEL.length > 0) {
                    ...
                    var compositeItemRec = (compositeItemRecId) ? nlapiLoadRecord('inventoryitem', compositeItemRecId, {
                        recordmode: 'dynamic',
                        customform: globeVar.custom_item_form
                    }) : nlapiCreateRecord('i​n​v​e​n​t​o​r​y​i​t​e​m', {
                        recordmode: 'dynamic',
                        customform: globeVar.custom_item_form
                    });
                    ...
                    for (var j = 0; j < components.length; j++) {
                        ...
                        if (componentObj.ITEM_NO === itemObj.ITEM_NO) {
                            ...
                            var componentItemRec = (componentItemRecId) ? nlapiLoadRecord(recType, componentItemRecId, {
                                recordmode: 'dynamic',
                                customform: globeVar.custom_item_form
                            }) : nlapiCreateRecord(recType, {
                                recordmode: 'dynamic',
                                customform: globeVar.custom_item_form
                            });
                            ...
                            var custComponentItemRec = (custComponentItemRecId) ? nlapiLoadRecord('customrecord_ts_bom', custComponentItemRecId, {
                                recordmode: 'dynamic'
                            }) : nlapiCreateRecord('customrecord_ts_bom', {
                                recordmode: 'dynamic'
                            });
                    ...
                    if (recType == 'lotnumberedinventoryitem') {
                        var itemRec = (itemRecId) ? nlapiLoadRecord(recType, itemRecId, {
                            recordmode: 'dynamic',
                            customform: globeVar.custom_item_form
                        }) : nlapiCreateRecord(recType, {
                            recordmode: 'dynamic',
                ...
                nlapiLogExecution('DEBUG', 'Filter for searching BPO', bpoObj.PO_NO);
                ...
                nlapiLogExecution('DEBUG', 'bpoRecId', bpoRecId);
nlapiLogExecution('DEBUG', 'XML Vendor', parseToString(bpoObj.VENDOR_NAME));

                filters[0] = new nlobjSearchFilter('entityid', null, 'is', parseToString(bpoObj.VENDOR_NAME));
                filters[1] = new nlobjSearchFilter('subsidiary', null, 'is', globeVar.subsidiary);
                var bpoVendor = recordLookup('vendor', filters, parseToString(bpoObj.VENDOR_NAME));
                ...
                nlapiLogExecution('DEBUG', 'factoryId', factoryId);

                // BEING BLANKET PO HEADER SETUP
                var bpoRec = (bpoRecId) ? nlapiLoadRecord('customrecord_ts_blanket_po', bpoRecId, {
                    recordmode: 'dynamic'
                }) : nlapiCreateRecord('customrecord_ts_blanket_po', {
                    recordmode: 'dynamic'
                });
                nlapiLogExecution('DEBUG', 'BPO PO NO', bpoObj.PO_NO);

                bpoRec.setFieldValue('custrecord_ts_bpo_subsidary', 6);
                ...
                nlapiLogExecution('DEBUG', 'Vendor Found', bpoVendor);
                ...
                //                bpoRec.setFieldValue('custrecord_ts_bpo_team', recordLookup('customrecord_ts_team_list', new nlobjSearchFilter('name', null, 'is', bpoObj.TEAM_NAME), bpoObj.TEAM_NAME, 'T'));
                ...
                // START Added by Clemen 20 Sept 2017
                if ((curLine.PURCHASE_ORDER).hasOwnProperty('BUYER_NAME') === true && curLine.PURCHASE_ORDER.BUYER_NAME) {
                    bpoRec.setFieldValue('custrecord_ts2_bpo_order_owner', recordLookup('employee', new nlobjSearchFilter('externalid', null, 'is', curLine.PURCHASE_ORDER.BUYER_NAME), curLine.PURCHASE_ORDER.BUYER_NAME));
                }

                if (bpoObj.hasOwnProperty('AE_NAME') === true && bpoObj.AE_NAME) {
                    bpoRec.setFieldValue('custrecord_ts2_bpo_ps_1', recordLookup('employee', new nlobjSearchFilter('externalid', null, 'is', bpoObj.AE_NAME), bpoObj.AE_NAME));
                }

                if (bpoObj.hasOwnProperty('ASSOCIATE_AE_NAME') === true && bpoObj.ASSOCIATE_AE_NAME) {
                    bpoRec.setFieldValue('custrecord_ts2_bpo_ps_2', recordLookup('employee', new nlobjSearchFilter('externalid', null, 'is', bpoObj.ASSOCIATE_AE_NAME), bpoObj.ASSOCIATE_AE_NAME));
                }

                var obLinkNoFilterExpression = [
                    ['tranid', 'is', bpoObj.OB_NUMBER], 'and', ['mainline', 'is', 'T'], 'and', ['subsidiary', 'is', '1']
                ];
                var obLinkNoResults = nlapiSearchRecord('purchaseorder', null, obLinkNoFilterExpression);
                var obLinkNoRecId = (obLinkNoResults) ? obLinkNoResults[0].getId() : null;

                if (obLinkNoRecId) {
                    bpoRec.setFieldValue('custrecord_ts2_bpo_ob_no_link', obLinkNoRecId);
                }
                nlapiLogExecution('DEBUG', 'Delivery to PO Number', bpoObj.DELIVERY_TO_PO_NUMBER);
                bpoRec.setFieldText('custrecord_ts2_bpo_del_to_po', bpoObj.DELIVERY_TO_PO_NUMBER);
                bpoRec.setFieldText('custrecord_ts_bpo_team', bpoObj.TEAM_NAME);
                // END Added by Clemen 20 Sept 2017


                // Commit record to database and log it to createdRecords array and Interface Log.
                bpoRecId = nlapiSubmitRecord(bpoRec, null, true);
                if(updateName == true){
                    nlapiSubmitField('customrecord_ts_blanket_po', bpoRecId, 'name', 'PO-' + bpoObj.PO_NO);
                }
                ...
                nlapiLogExecution('DEBUG', 'bpoLineRecId', bpoLineRecId);

                // BEGIN BLANKET PO LINE SETUP
                var cItemObj = searchTsItemByItemName(bpoLineObj.ITEM_NUMBER); // Added by Clemen 19 Sept 2017

                var bpoLineRec = (bpoLineRecId) ? nlapiLoadRecord('customrecord_ts_blanket_po_line', bpoLineRecId, {
                    recordmode: 'dynamic'
                }) : nlapiCreateRecord('customrecord_ts_blanket_po_line', {
                    recordmode: 'dynamic'
                });
                ...
                bpoLineRec.setFieldValue('custrecord_ts_bpol_item', cItemObj.id); // Modified by Clemen 19 Sept 2017
                ...
                bpoLineRec.setFieldValue('custrecord_ts_bpol_ms_edit_unit_price', (isNaN(bpoLineObj.MS_EDI_UNIT_PRICE)) ? '' : bpoLineObj.MS_EDI_UNIT_PRICE);
                ...
                bpoLineRec.setFieldValue('custrecord_ts_bpol_ms_edi_quantity', (isNaN(bpoLineObj.MS_EDI_QUANTITY)) ? '' : bpoLineObj.MS_EDI_QUANTITY);
                bpoLineRec.setFieldValue('custrecord_ts_bpol_list_price_per_unit', (isNaN(bpoLineObj.LIST_PRICE_PER_UNIT)) ? '' : bpoLineObj.LIST_PRICE_PER_UNIT);
                bpoLineRec.setFieldValue('custrecord_ts_add_charge_per_unit', (isNaN(bpoLineObj.ROYALTY_CHARGES_UNIT)) ? '' : bpoLineObj.ROYALTY_CHARGES_UNIT);
                if (bpoLineObj.hasOwnProperty('ROYALTY_CHARGES_PERC')) {
                ...
                // START Added by Clemen - 20 Sept 2017
                bpoLineRec.setFieldValue('custrecord_ts2_bpol_expected_recpt_dd', formatDate(bpoLineObj.CUSTOMER_NEED_BY_DATE));
                bpoLineRec.setFieldValue('custrecord_ts2_bpol_cust_need_by_dd', formatDate(bpoLineObj.CUSTOMER_NEED_BY_DATE));
                // EEND Added by Clemen - 20 Sept 2017
                ...
                nlapiLogExecution('DEBUG', 'START RELEASE PO', 'bpoVendor: ' + bpoVendor);
                nlapiLogExecution('DEBUG', 'START RELEASE PO', 'rspoName: ' + rspoName);
                nlapiLogExecution('DEBUG', 'START RELEASE PO', 'rspoNameDupe: ' + rspoNameDupe);
                var rspoRec;
                ...
                if (bpoVendor == null) {
                    ...
                    interfaceRec.setFieldValue('custrecord_interface_complete', 'F');
                    interfaceRec.setFieldValue('custrecord_has_error', 'T');
                    nlapiLogExecution('DEBUG', 'bpoVendor', bpoVendor);

                } else {

                    /****** START ADDED by CLEMEN (07-Sept-2017) *******/
                    var rhRecId = recordLookup('customrecord_ts2_rlpo', new nlobjSearchFilter('name', null, 'is', ('RL-' + bpoObj.PO_NO + '-' + curLine.ASN_LINE_NUM)), 'RL-' + bpoObj.PO_NO + '-' + curLine.ASN_LINE_NUM, null, 'T');

                    nlapiLogExecution('DEBUG', 'rhRecId', 'checking');

                    nlapiLogExecution('DEBUG', 'rhRecId', rhRecId);

                    var rhRec = (rhRecId) ? nlapiLoadRecord('customrecord_ts2_rlpo', rhRecId, {
                        recordmode: 'dynamic'
                    }) : nlapiCreateRecord('customrecord_ts2_rlpo', {
                        recordmode: 'dynamic'
                    });
                    rhRec.setFieldValue('custrecord_ts_bpo_subsidary', 6);
                    rhRec.setFieldValue('custrecord_ts2_rlpo_dd', formatDate(rspoObj.PO_CREATED_DATE));
                    rhRec.setFieldValue('custrecord_ts2_rlpo_bpo_no', bpoRecId);
                    rhRec.setFieldValue('custrecord_ts2_rlpo_release_no', rspoObj.RELEASE_NUM);//curLine.ASN_LINE_NUM);
                    rhRec.setFieldValue('custrecord_ts2_rlpo_pj', projRecId);
                    rhRec.setFieldValue('custrecord_ts2_rlpo_ship_to', recordLookup('customrecord_ts_asn_end_customer_list', new nlobjSearchFilter('name', null, 'is', rspoObj.SHIP_TO), rspoObj.SHIP_TO));
                    rhRec.setFieldValue('custrecord_ts2_rlpo_supplier', recordLookup('vendor', filters, parseToString(bpoObj.VENDOR_NAME)));
                    rhRec.setFieldValue('custrecord_ts2_rlpo_fty', recordLookup('customrecord_fty_profile', new nlobjSearchFilter('custrecord_ts_fty_number', null, 'is', bpoObj.FACTORY_ID), bpoObj.FACTORY_ID));
                    rhRec.setFieldValue('custrecord_ts2_rlpo_pyt_terms', paymentTerm);
                    rhRec.setFieldValue('custrecord_ts2_rlpo_status', 9); // Pre-approved
                    rhRec.setFieldValue('custrecord_ts2_rlpo_ms_end_cus', rspoObj.MS_END_CUSTOMER_NAME);
                    rhRec.setFieldValue('custrecord_ts2_rlpo_price_sticker', bpoLineObj.UNIT_PRICE);

                    rhRec.setFieldValue('custrecord_ts2_rlpo_order_owner', recordLookup('employee', new nlobjSearchFilter('externalid', null, 'is', curLine.PURCHASE_ORDER.BUYER_NAME), curLine.PURCHASE_ORDER.BUYER_NAME));

                    rhRecId = nlapiSubmitRecord(rhRec, true, true);
                    /****** END ADDED by CLEMEN (07-Sept-2017) *******/

                    var filterExpression = [
                        ['tranid', 'is', rspoName], 'and', ['mainline', 'is', 'T']
                    ];
                    ...
                    nlapiLogExecution('DEBUG', 'BEGIN PURCHASE ORDER SETUP', rspoRecId);

                    if (rspoRecId) {
                        rspoRec = nlapiLoadRecord('purchaseorder', rspoRecId, {
                            recordmode: 'dynamic',
                            customform: globeVar.custom_rspo_form
                        });
                        nlapiLogExecution('DEBUG', 'externalid vs rspoNameDupe', rspoRec.getFieldValue('externalid') + ' vs ' + rspoNameDupe);
                        if (rspoRec.getFieldValue('externalid') != rspoNameDupe) {
                            ...
                            if (rspoRecId) {
                                ...
                                rspoRec = nlapiCreateRecord('purchaseorder', {
                                    recordmode: 'dynamic',
                                    customform: globeVar.custom_rspo_form
                                });
                                rspoRec.setFieldValue('custbodyts_rspo_bpo_no', bpoRecId);
                        ...
                        var rspolQty = parseInt(rspoRec.getFieldValue('custbody_ts2_rspol_qty')) || 1;

                        rspoRec.setFieldValue('custbody_ts2_rspol_qty', rspolQty);

                        nlapiLogExecution('DEBUG', 'Amount', rspoRec.getFieldValue('custbody_ts2_rspol_amt'));

                    } else {
                        //                          rspoRec = nlapiCreateRecord('purchaseorder', {recordmode: 'dynamic', customform: globeVar.custom_rspo_form});
                        rspoRec = new nlapiCreateRecord('purchaseorder', {
                            recordmode: 'dynamic',
                            customform: globeVar.custom_rspo_form
                        });
                        rspoRec.setFieldValue('custbodyts_rspo_bpo_no', bpoRecId);
                        ...
                        rspoRec.setFieldValue('tranid', rspoNameDupe);
                        rspoRec.setFieldValue('externalid', rspoNameDupe);
                        nlapiLogExecution('DEBUG', 'RSPO Name Dupe', rspoNameDupe)

                        rspoRec.setFieldValue('custbody_ts2_rspol_qty', curLine.ACTUAL_QUANTITY);
                        nlapiLogExecution('DEBUG', 'Qty', rspoRec.getFieldValue('custbody_ts2_rspol_qty'));
                    }
                //}

                nlapiLogExecution('DEBUG', 'start', '1');

                /****** START ADDED by CLEMEN (08-Sept-2017) *******/
                rspoRec.setFieldValue('custbody_ts2_rspol_bpol_no', bpoLineRecId);
                rspoRec.setFieldValue('custbody_ts2_rspol_rlpo_no', rhRecId);

                rspoRec.setFieldValue('custbody_ts_rspo_att_suggested_price', bpoLineObj.UNIT_PRICE);
                ...
                rspoRec.setFieldValue('custbody_ts2_rspol_item', cItemObj.id);
                rspoRec.setFieldValue('custbody_ts2_rlpol_item_name', cItemObj.name);

                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_rel_item_no', cItemObj.id);
                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_rel_item_desc', cItemObj.name);
                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_brand_name', rspoLineObj.MS_BRAND_NAME);
                rspoRec.setFieldValue('custbody_ts2_rspol_color', rspoLineObj.COLOR);
                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_package_type', rspoLineObj.MS_PACKAGE_TYPE);
                //              rspoRec.setFieldValue('custbody_ts2_rlpol_ms_cons_id', releaseLine.custcol_consolidated_id);
                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_upc', rspoLineObj.MS_UPC);
                //              rspoRec.setFieldValue('custbody_ts2_rlpol_hts_code', releaseLine.custcol_hts);
                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_sscc', rspoLineObj.MS_SSCC);
                rspoRec.setFieldValue('custbody_ts2_rlpol_ms_spec_rfq', rspoLineObj.MS_SPECIAL_REQ);
                /****** END ADDED by CLEMEN (08-Sept-2017) *******/
                ...
                rspoRec.setFieldValue('custbody_ts_rspo_title_transfer', recordLookup('customrecord_ts_rspo_title_tsfr_list', new nlobjSearchFilter('name', null, 'is', rspoObj.TITLE_TRANSFER), rspoObj.TITLE_TRANSFER, 'T'));
                rspoRec.setFieldValue('custbody_ts_rspo_consignee', recordLookup('customrecord_ts_asn_consignee_list', new nlobjSearchFilter('name', null, 'is', rspoObj.CONSIGNEE), rspoObj.CONSIGNEE, 'T'));
                ...
                rspoRec.setFieldValue('custbody_ts_rspo_notify_party', recordLookup('customrecord_ts_rspo_notify_party_list', new nlobjSearchFilter('name', null, 'is', rspoObj.NOTIFY_PARTY), rspoObj.NOTIFY_PARTY, 'T'));
                rspoRec.setFieldValue('custbody_ts_rspo_notify_party_2', recordLookup('customrecord_ts_rspo_notify_party_list', new nlobjSearchFilter('name', null, 'is', rspoObj.NOTIFY_PARTY2), rspoObj.NOTIFY_PARTY2, 'T'));
                rspoRec.setFieldValue('custbody_ts_rspo_notify_party_3', recordLookup('customrecord_ts_rspo_notify_party_list', new nlobjSearchFilter('name', null, 'is', rspoObj.NOTIFY_PARTY3), rspoObj.NOTIFY_PARTY3, 'T'));
                ...
                rspoRec.setFieldValue('custbody_ts_rspo_port_of_discharge', recordLookup('customrecord_ts_rspo_asn_pod_list', new nlobjSearchFilter('name', null, 'is', rspoObj.PORT_OF_DISCHARGE), rspoObj.PORT_OF_DISCHARGE, 'T'));
                rspoRec.setFieldValue('custbody_ts_rspo_forwarder', recordLookup('customrecord_ts_asn_forwarder_list', new nlobjSearchFilter('name', null, 'is', rspoObj.FORWARDER), rspoObj.FORWARDER, 'T'));
                ...
                nlapiLogExecution('DEBUG', 'Rel Line Num', rspoLineObj.REL_LINE_NUM);
                nlapiLogExecution('DEBUG', 'bpo no', bpoRecId);
                nlapiLogExecution('DEBUG', 'bpo line no', bpoLineRecId);
                ...
                rspoRec.setCurrentLineItemValue('item', 'item', cItemObj.id); // Changed by Clemen 8-Sept-2017
                ...
                nlapiLogExecution('DEBUG', 'Quantity', curLine.ACTUAL_QUANTITY);
                nlapiLogExecution('DEBUG', 'Rate', bpoLineObj.UNIT_PRICE);
                ...
                //              rspoRec.setCurrentLineItemValue('item', 'taxcode', '41610'); // Added by Clemen 8-Sept-2017

                rspoRec.commitLineItem('item');
                // END PURCHASE ORDER LINE SETUP
                nlapiLogExecution('DEBUG', 'Committed Line Item', 'End');
                // Commit record to database and log it to createdRecords array and Interface Log.
                rspoRecId = nlapiSubmitRecord(rspoRec, false, true);
            }
                ...
                nlapiLogExecution('DEBUG', 'ASN LINE Rec ID', asnLineRecId);
                // BEGIN ASN LINE SETUP
                var asnLineRec = (asnLineRecId) ? nlapiLoadRecord('customrecord_ts_asn_item_details', asnLineRecId, {
                    recordmode: 'dynamic'
                }) : nlapiCreateRecord('customrecord_ts_asn_item_details', {
                    recordmode: 'dynamic'
                });
                ...
                //              asnLineRec.setFieldValue('custrecord_ts_asn_item', cItemObj.id); // Added by Clemen 8-Sept-2017
                ...
                // Commit record to database and log it to createdRecords array and Interface Log.
                if(asnLineRecId && asnLineRec.getFieldValue('custrecord_ts2_asn_item_status') == '2'){
                    nlapiLogExecution('DEBUG', 'ASN Status','ASN is already closed');
                }else{
                ...
                if (!isEmpty(containers)) {
                    for (var l = 0; l < containers.length; l++) {
                        ...
                        containerRecId = recordLookup('customrecord_container_details', new nlobjSearchFilter('externalid', null, 'is', containerName), containerName, null, 'T');
                        var containerRec = (containerRecId) ? nlapiLoadRecord('customrecord_container_details', containerRecId, {
                            recordmode: 'dynamic'
                        }) : nlapiCreateRecord('customrecord_container_details', {
                            recordmode: 'dynamic'
                        });
                        ...
                        containerRec.setFieldValue('custrecord_ts2_ctnr_asn_no_hidden', asnRecId); //by jeff. 15/11/2017
                    ...
                    nlapiLogExecution('DEBUG', 'CONTAINER', 'No Container Found');
            ...
            if (interfaceRec.getFieldValue('custrecord_has_error') === 'T') {
                nlapiLogExecution('ERROR', 'ERROR FOUND', 'Setting ASN Status to ERROR');
...
function formatDate(date) {
    if (date) {
        ...
        //        var newDate = day + '/' + month + '/' + dateArr[2];
...
//var day = dateArr[0].replace(/^0+/, '');
//var month = toTitleCase(dateArr[1]);
...
recordIndex = {
    ...
    'customrecord_ts_asn_consignee_list': 'Consignee list',
    'customrecord_ts_asn_forwarder_list': 'Forwarder list',
    ...
    'customrecord_ts_asn_pol_list': 'Port of Loading list',
    'customrecord_ts_rspo_asn_pod_list': 'Port of Discharge list',
    ...
    'customrecord_ts_rspo_title_tsfr_list': 'Title Transfer list',
    'customrecord_ts_rspo_notify_party_list': 'Notify Party list',
...
function searchTsItemByItemName(itemName) {
    var tsItemObj = null;

    if (itemName) {
        var filters = [];
        filters.push(new nlobjSearchFilter('itemid', null, 'is', itemName));
        filters.push(new nlobjSearchFilter('subsidiary', null, 'is', 6));

        var columns = [];
        columns.push(new nlobjSearchColumn('custitem_ts_item_customer_item_no'));
        ...
        if (searchResults && searchResults.length > 0) {
            tsItemObj = {
                ...
                name: searchResults[0].getValue('custitem_ts_item_customer_item_no')
            };

            nlapiLogExecution('DEBUG', 'Item found!', tsItemObj.id + " : " + tsItemObj.name);
        }
    }
...
function isFileExists(fileId) {
    return nlapiSearchRecord('file', null, [new nlobjSearchFilter('internalid', null, 'is', fileId)]) != null;
}
