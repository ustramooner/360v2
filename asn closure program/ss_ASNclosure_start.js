...
function processASN_Batch(type) {
	try {
		
		dLog(LOG_NAME, '>>>START<<<');
		
		var rs = nlapiSearchRecord('customrecord_ts_asn', SS_ASN2PROCESS);
		...
		for (var i = 0; i < rs.length; i++) {
			...
			var asnReset = rs[i].getValue('custrecord_asn_reset');

			dLog(LOG_NAME, 'ASN Reset = ' + asnReset);

			if (asnReset == 'F') {
				dLog(LOG_NAME, 'Reset is not checked. Next ASN.');
				continue;
			}  
