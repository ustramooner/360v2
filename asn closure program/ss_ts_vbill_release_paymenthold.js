...
function vbill_release_paymenthold() {
	...
	try {
			...
			var sc_vb_resultIndex = 0;
			var sc_vb_resultStep = 100;
		    ...
		    var sc_vb_searchRun = nlapiLoadSearch( 'vendorbill', 'customsearch_one_vb_srr_search' );

		    var sc_vb_resultSet = sc_vb_searchRun.runSearch();

			do {
		    var sc_vb_searchResults = sc_vb_resultSet.getResults( sc_vb_resultIndex, sc_vb_resultIndex + sc_vb_resultStep );
			for ( var v = 0; sc_vb_searchResults != null && v < sc_vb_searchResults.length; v ++ ) {
			      ...
			      nlapiLogExecution('DEBUG', 'remaining usage', context.getRemainingUsage()  + " sc_vb_resultSet v:" + v );
			...
			sc_vb_resultIndex = sc_vb_resultIndex + sc_vb_resultStep;
         nlapiYieldScript();
		} while (sc_vb_searchResults.length > 0);
