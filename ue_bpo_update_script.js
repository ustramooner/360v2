      ...
      function getRecord(recId, child){
        ...
        function afterSubmit(context) {
            ...
            if (context.type === 'edit' || context.type === 'xedit') {
                ...
                log.debug('Record', 'Record Type: ' + rec.type + ' Record Id: ' + rec.id);

                var parentExists = false;

                //if(rec.type === 'customrecord_ts_blanket_po_line'){

                var parents = recordParent[rec.type] || [];
                ...
                //}
                              ...
                              var isRecordExists = recordExists(rec.type, rec.id);
                
                log.debug('record already exists?', isRecordExists);

                if(!isRecordExists){
                  
                  var results = getDependencies([], rec.type, rec.id);
                  
                    if(results && results.length > 0){
                        var tmpRec = record.create({
                            type: 'customrecord_ts2_recordupdater_tmprec',
                            isDynamic: true
                        });
                        tmpRec.setValue('custrecord_rectype', rec.type);
                        tmpRec.setValue('custrecord_recid', rec.id);
                        tmpRec.setValue('custrecord_json_record', JSON.stringify(results));
                        tmpRec.save(true,false);
                    }
                }
